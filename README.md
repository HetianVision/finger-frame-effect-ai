...
3. **Composite** — the AI video is revealed through the tracked quad with
   the dashed marching-ants outline and pulsing corner dots. A face-based
   auto-alignment step (MediaPipe Face Landmarker on both videos) corrects
   the scale/position drift AI models introduce, so the animated face sits
   exactly on the real one at the window boundary.
